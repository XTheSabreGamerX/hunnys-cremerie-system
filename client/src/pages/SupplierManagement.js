...
const SupplierManagement = () => {
	...
	const validateFormData = (data) => {
		...
		if (
			modalMode === 'add' &&
			suppliers.some(s => s.supplierId.toLowerCase() === data.supplierId.toLowerCase())
		) {
	...
	const saveSupplier = async (data) => {
		try {
			...
			setModalMode('view');
			showPopup(`Supplier ${modalMode === 'add' ? 'added' : 'updated'} successfully!`);
		} catch (err) {
			...
			{isViewOpen && viewedSupplier && (
				<ViewModal
					...
					fields={[
						...
						{ name: 'createdAt', label: 'Created', formatter: (val) => new Date(val).toLocaleString() },
						{ name: 'updatedAt', label: 'Updated', formatter: (val) => new Date(val).toLocaleString() },
					]}
					onClose={() => {
						setIsViewOpen(false);
